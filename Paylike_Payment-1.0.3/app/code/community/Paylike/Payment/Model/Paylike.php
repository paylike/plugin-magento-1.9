    ...
    public function authorize(Varien_Object $payment, $amount, $ajax = false)
    {
        ...
        if (empty($apiKey)) {
            if ($ajax) {
                ...
                $errormsg = $this->__('The API key is not valid.');
            ...
            if (!empty($fetch['transaction'])) {
                ...
                if ($transaction['amount'] != Mage::helper('paylike_payment/currencies')->Ceil($order->getGrandTotal(),$order->getOrderCurrencyCode()) || Mage::app()->getStore()->getCurrentCurrencyCode() != $transaction['currency'] || $transaction['custom']['quoteId'] != $quoteId) {
                    if ($ajax) {
                        ...
                        $errormsg = $this->__('The transaction is not valid.');
                        Mage::throwException($errormsg);
                    }
                } else {
    ...
    public function capture(Varien_Object $payment, $amount, $ajax = false)
    {
        if ($amount <= 0) {
            if ($ajax) {
                ...
                $errormsg = $this->__('The amount is not valid for capture.');
                Mage::throwException($errormsg);
            }
        }
        ...
        if (!empty($paylike_admin) && $paylike_admin['captured'] == 'NO') {
                ...
                if (!empty($capture['transaction'])) {
                    ...
                    if (!empty($capture[0]['message'])) {
                        ...
                        if ($ajax) {
                            ...
                            $errormsg = $this->__('The transaction has failed.');
                            Mage::throwException($errormsg);
                        }
                    }
                }
            ...
            if ($ajax) {
                ...
                $errormsg = $this->__('The order was already captured.');
                Mage::throwException($errormsg);
            }
        } else {
            if ($ajax) {
                ...
                $errormsg = $this->__('The transaction is not valid.'.$payment->getPaylikeTransactionId());
                Mage::throwException($errormsg);
            }
        }
        return $this;
        ...
        if (!empty($paylike_admin) && $paylike_admin['captured'] == 'YES') {
            ...
            if ($amount <= 0) {
                if ($ajax) {
                    ...
                    $errormsg = $this->__('The amount you entered for refund is not valid.');
                    Mage::throwException($errormsg);
                }
            }
            Paylike\Client::setKey($this->getApiKey());
            ...
            if ($ajax) {
                ...
                $errormsg = $this->__('In order to refund you first need to capture the transaction.');
                Mage::throwException($errormsg);
            }
        } else {
            if ($ajax) {
    ...
    public function void(Varien_Object $payment, $ajax = false)
    {
        ...
        if (!empty($paylike_admin) && $paylike_admin['captured'] == 'NO') {
            ...
            if ($ajax) {
                ...
                $errormsg = $this->__('You can\'t void the transaction because it has already been captured, you can only refund.');
                Mage::throwException($errormsg);
            }
        } else {
            if ($ajax) {
