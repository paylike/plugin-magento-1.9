...
class Paylike_Paymentgateway_IndexController extends Mage_Core_Controller_Front_Action
{
    ...
    public function paymentAction()
    {
        ...
        switch ($paylike_action) {
            case 'capture' :
                $response = Mage::getModel('paymentgateway/paymentgateway')->capture($orderObject->getPayment(), $order_data['grand_total'], true);
                break;
            case 'refund' :
                ...
                $response = Mage::getModel('paymentgateway/paymentgateway')->refund($orderObject->getPayment(), $paylike_amount_to_refund, $paylike_refund_reason, true);
                break;
            case 'void' :
                $response = Mage::getModel('paymentgateway/paymentgateway')->void($orderObject->getPayment(), true);
                break;
        }
        ...
        if (empty($logo_name)) {
            ...
            if (!empty($paylike_logos)) {
                ...
                if (!empty($_FILES['logo_file']['name'])) {
                    //$target_dir = Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_SKIN) . 'frontend/paylike/logos';
                    $target_dir = Mage::getBaseDir('skin') . '/frontend/paylike/logos/';
                    ...
                    if (file_exists($target_file)) {
                        ...
                        if ($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg"
                            ...
                            if (move_uploaded_file($_FILES['logo_file']["tmp_name"], $target_file)) {
                                ...
                                $paylikeLogosModel = Mage::getModel('paymentgateway/paylikelogos');
                                try {
                                    ...
                                    if ($res) {
                                        ...
                                        $response = array(
                                            'status' => 0,
                                            'message' => "Oops! An error occurred while save logo."
                                        );
                                        $jsonData = Mage::helper('core')->jsonEncode($response);
                                        $this->getResponse()->setHeader('Content-type', 'application/json');
                                        $this->getResponse()->setBody($jsonData);
    ...
    {
        $pop_up_title = Mage::getStoreConfig('payment/paymentgateway/pop_up_title');
        if ($pop_up_title != '') {
            echo $pop_up_title;
        } else {
